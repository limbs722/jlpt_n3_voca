...
export const LoginButton = () => {
  ...
  if (isLoggedIn && user) {
    return (
      <AvatarButton onClick={signOut} title="로그아웃">
        {user.user_metadata?.avatar_url ? (
          <AvatarImg
            src={user.user_metadata.avatar_url as string}
            alt={user.user_metadata?.full_name ?? '유저'}
            width={28}
            height={28}
          />
        ) : (
  ...
  return (
    <SignInButton onClick={signInWithGoogle} aria-label="Google로 로그인">
      <GoogleIcon />
      로그인
    </SignInButton>
  );
